...
require('config.inc.php'); // get the db connection info
...
$query=mysql_query("SELECT total_votes, total_value FROM $rating_dbname.$rating_tableName WHERE id='$id' ")or die(" Error: ".mysql_error());
...
if ($static == 'static') {
      ...
      $rater.='>Beoordeling: <strong> '.$rating1.'</strong>/'.$units.' ('.$count.' '.$tense.')';
      $rater.='  </p>';
      $rater.='</div>';
      return $rater;
 }
