...
if(isset($_POST['save']))
{
   ...
   $query = "UPDATE page SET title='$title', id_menu='$content' WHERE id='$id'";
...
<?php
open();
$nodes = mysql_query("SELECT * FROM menu");
close();
while($node = mysql_fetch_array($nodes)){
?>
<option value="<?=$node[id];?>"<?php if($row[id_menu] == $node[id]) echo ' selected="yes"';?>><?=$node[title];?></option>
<?php
}?>
</select></td>
</tr>
