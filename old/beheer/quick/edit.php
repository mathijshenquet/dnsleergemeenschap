...
if(isset($_GET['id']))
{
   $query  = "SELECT * FROM quick WHERE id = '{$_GET['id']}'";
...
if(isset($_POST['save']))
{
   ...
   $query = "UPDATE quick SET qtitle='$title', id_node='$content' WHERE id='$id'";
...
<td><input name="qtitle" type="text" class="box" id="title" value="<?=$row[qtitle];?>"></td>
</tr>
...
?>
<option value="<?=$node[id];?>"<?php if($row[id_node] == $node[id]) echo ' selected="yes"';?>><?=$node[title];?></option>
<?php
}?>
</select></td>
</tr>
